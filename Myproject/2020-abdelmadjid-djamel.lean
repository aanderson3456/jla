...
def Ω (n:ℕ) : ℕ := (Nat.factors n).length
...
example : Nat.factors 6 = [2,3] := by decide

#eval Nat.factors 1001

#eval padicValNat 2 16777216

#eval padicValNat 5 10

example : List.Perm [3,2] (Nat.factors 6) := by decide

example : 24 ≤ padicValNat 2 16777216 ↔ (List.replicate 24 2).Subperm
  (Nat.factors 16777216) :=
    le_padicValNat_iff_replicate_subperm_factors Nat.prime_two (Ne.symm (Nat.zero_ne_add_one 16777215))

example : (List.replicate 4 2).Subperm
  (Nat.factors 16) := by decide
