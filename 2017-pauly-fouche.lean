...
lemma de_d : (de x y).toReal = d x y := by rfl
...
lemma edist_of_diff {t f : ℕ → Bool} (h : t k ≠ f k) :
  (1/2)^k ≤ edist t f := by
...
lemma sup_edist_lower_bound'1 (S : Set (ℕ → Bool)) (t f : ℕ → Bool)
  (h₀ : t k ≠ f k) (h₁ : t ∈ S ∧ f ∈ S)
...
lemma sup_dist_lower_bound : 1 ≤ (⨆ x ∈ Set.univ, ⨆ y ∈ Set.univ, d x y) := by
  ...
  have Q₁: F tr ≤ sSup (Set.range F) := by
    ...
    . exists 1; exists tr; apply le_antisymm; exact sup_dist_bound _; exact le_of_eq_of_le dist_tf Q₀
...
lemma toReal_sup_sup_eq_sup_toReal_sup' : (⨆ x ∈ Set.univ, ⨆ y ∈ Set.univ, @edist (ℕ → Bool) _ x y).toReal
...
lemma half_nonneg : 0 ≤ ((1:ℝ)/2)^(k:ℕ) := by
  apply pow_nonneg
...
lemma NNReal_proof :
    ENNReal.ofNNReal ((⟨((1:ℝ)      /2)^(k:ℕ),half_nonneg_k⟩ : NNReal))
  ...
  ⨆ y ∈ {x | x 0 = b}, edist x y ≤ 1 / 2 := by
              ...
              . have : (1:ENNReal)/2 = ((1:ENNReal)/2)^1 := by exact Eq.symm (pow_one (1 / 2))
                ...
                suffices PiNat.firstDiff x y ≥ 1 by
                  let W := @pow_le_pow_of_le_one
                    ENNReal _ (1/2) (by simp) (by simp) 1 (PiNat.firstDiff x y) this
